...
def calculate_sun_times(start_date: datetime, num_days: int) -> pd.DataFrame:
    ...
    """
    data_print = []  # Data for readable output
...
def main() -> None:
    ...
    for _, row in dunix.iterrows():
         print(f"{row['Date (unix)']}, {row['Sunrise (Unix ms)']}, {row['Sunset (Unix ms)']}, {row['Day Length (ms)']} , {row['Sunrise (local)']}, {row['Sunset (local)']}, {row['Day Length']}")
